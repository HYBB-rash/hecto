...
impl StatusMessage {
    fn from(message: String) -> Self {
        Self { time: Instant::now(), text: message }
    }
}
...
impl Editor {
    ...
    pub fn default() -> Self {
        ...
        let mut initial_status = String::from("HELP: Ctrl-O = quit");
    ...
    fn move_cursor(&mut self, key: Key) {
        ...
        match key {
            ...
            Key::PageDown => {
                y = if y.saturating_add(terminal_height) < height { y + (terminal_height as usize) } else { 0 };
            }
            Key::Home => {
                x = 0;
            }
